...
import numpy as np
from WinForm.ScaleConvertion.PicProcess.DataPredict import predict
from scipy.optimize import minimize

from WinForm.CommonObject.Build import logger
from WinForm.CommonObject.TmpFile import Tmp
from WinForm.DataProcess import SedProcess
...
class Predict:
    ...
    def concate(self):
        ...
        :return:
        """
        if self.ans_nir is not None:
            sm = Smooth(self.ans_vis, self.ans_nir, self.index)
            self.ans = sm.ans
        else:
            self.ans = self.ans_vis
        return
